...
load_dotenv()
web3_rpc_url = os.getenv('MUMBAI_RPC_URL')
w3 = Web3(HTTPProvider(web3_rpc_url))
path = os.getcwd()

# Define the contract ABI and address
with open(path + '/contracts_mumbai/desci/DeSciPrint.json', 'r') as f:
    desci_abi_file = json.load(f)

with open(path + '/contracts_mumbai/preprint/PrePrintTrack.json', 'r') as f:
...
def create_desci_listeners(loop):
    ...
    with open(path + '/contracts_mumbai/desci/contract-address.json', 'r') as f:
...
def create_preprint_listeners(loop):
    # loop = asyncio.get_running_loop()
    with open(path + '/contracts_mumbai/preprint/contract-address.json', 'r') as f:
        address_file = json.load(f)
